...
WITH ProductStats AS (
    ...
    GROUP BY od.ProductID, p.ProductName, o.ShipCity
),
TopProducts AS (
    SELECT TOP 5
        ProductID,
        ProductName,
        SUM(CityQuantity) as TotalQuantity,
        AVG(AvgPrice) as AvgPrice
    FROM ProductStats
    GROUP BY ProductID, ProductName
    ORDER BY SUM(CityQuantity) DESC
),
TopCityPerProduct AS (
    SELECT 
        ...
        ps.ShipCity,
        ps.CityQuantity,
        ROW_NUMBER() OVER (PARTITION BY ps.ProductID ORDER BY ps.CityQuantity DESC) as rn
    FROM ProductStats ps
    WHERE ps.ProductID IN (SELECT ProductID FROM TopProducts)
)
SELECT 
    tp.ProductID,
    tp.ProductName,
    tp.AvgPrice,
    tp.TotalQuantity,
    tc.ShipCity as TopCustomerCity,
    tc.CityQuantity as CityQuantity
FROM TopProducts tp
JOIN TopCityPerProduct tc ON tp.ProductID = tc.ProductID
WHERE tc.rn = 1
